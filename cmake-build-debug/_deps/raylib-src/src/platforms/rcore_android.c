...
typedef struct {
    ...
    bool appEnabled;                    // Flag to detect if src is active ** = true
...
void OpenURL(const char *url)
{
    ...
    else
    {
        ...
        jclass activityClass = (*env)->FindClass(env, "android/src/Activity");
...
void PollInputEvents(void)
{
    ...
    while ((pollResult = ALooper_pollAll(platform.appEnabled? 0 : -1, NULL, &pollEvents, (void**)&platform.source)) >= 0)
    {
        ...
        if (platform.app->destroyRequested != 0)
        {
            ...
            //ANativeActivity_finish(platform.src->activity);
        }
    }
}
...
int InitPlatform(void)
{
    ...
    //AConfiguration_getDensity(platform.src->config);
    //AConfiguration_getKeyboard(platform.src->config);
    //AConfiguration_getScreenSize(platform.src->config);
    //AConfiguration_getScreenLong(platform.src->config);
    ...
    while (!CORE.Window.ready)
    {
        ...
        while ((pollResult = ALooper_pollAll(0, NULL, &pollEvents, (void**)&platform.source)) >= 0)
        {
            ...
            // NOTE: Never close window, native activity is controlled by the system!
            //if (platform.src->destroyRequested != 0) CORE.Window.shouldClose = true;
        }
    }
...
static int InitGraphicsDevice(void)
{
    ...
    //ANativeWindow_setBuffersGeometry(platform.src->window, 0, 0, displayFormat);       // Force use of native display size
...
static void AndroidCommandCallback(struct android_app *app, int32_t cmd)
{
    switch (cmd)
    {
        ...
        case APP_CMD_TERM_WINDOW:
        {
            ...
            // NOTE 1: This case is used when the user exits the src without closing it. We detach the context to ensure everything is recoverable upon resuming.
        ...
        {
            //AConfiguration_fromAssetManager(platform.src->config, platform.src->activity->assetManager);
            //print_cur_config(platform.src);
...
static int32_t AndroidInputCallback(struct android_app *app, AInputEvent *event)
{
    ...
    else if (type == AINPUT_EVENT_TYPE_KEY)
    {
        ...
        {
            // Let the OS handle input to avoid src stuck. Behaviour: CMD_PAUSE -> CMD_SAVE_STATE -> CMD_STOP -> CMD_CONFIG_CHANGED -> CMD_LOST_FOCUS
